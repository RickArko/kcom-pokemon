"""End-to-end training pipeline for Pokemon TCG AI Battle Challenge Strategy.
...
from pokemon.models import SimpleAverageMeta, StackingEnsemble, save_submission
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Pokemon TCG strategy prediction ensemble.")
    parser.add_argument(
        "--config",
        type=str,
...
def main() -> None:
    ...
    logger.info("=" * 60)
    logger.info("Pokemon TCG Battle Strategy — Training Pipeline")
    logger.info("=" * 60)
    ...
    with track_experiment(cfg, run_name=args.run_name) as run:
        ...
        save_submission(test["id"], test_preds, str(run.submission_path), target_col=target_col)
        logger.info("  Submission saved to %s", run.submission_path)

        canonical_sub = Path(cfg["paths"]["submissions"]) / "submission.csv"
        save_submission(test["id"], test_preds, str(canonical_sub), target_col=target_col)
        logger.info("  Canonical submission → %s", canonical_sub)

    elapsed = time.time() - t0
    logger.info("[5/5] Done in %.1fs", elapsed)
    logger.info("  OOF accuracy (stacked): %.4f", ensemble.overall_oof_score_)
