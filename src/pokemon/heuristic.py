"""Lucario heuristic agent — state-aware rule-based Pokemon TCG agent.

Parses the full ``cg`` observation via :mod:`pokemon.state`, looks up card/attack
data via :mod:`pokemon.card_db`, and selects actions with these priorities:
...
This module is part of the ``pokemon`` package so it is bundled in Kaggle
submissions.  Experiment agents (exp002, exp003, exp005) import from here.
"""
