"""exp009_baseline — sample-tuned Lucario deck + Full heuristic MCTS + opponent classifier.

Self-contained Kaggle submission agent.  Wraps the bundled
``pokemon.heuristic_phase.TwoHKOBenchAgent`` heuristic with MCTS, using an
opponent classifier to pass the real opponent deck to the search.  The deck
(``deck.csv``) is the data-driven meta-aligned Lucario build (see
SESSION_NOTES.md): adds Poke Pad (the #1 meta staple, 86.8% of decks) to the
sample-tuned build, fixes the 3-ACE-SPEC bug, keeps Maximum Belt.

Improvements over exp003:
1. Full heuristic rollout (deterministic) via TwoHKOBenchAgent (prize/phase/2HKO).
2. Opponent classifier — passes the real opponent deck to MCTS once classified.
3. Meta-aligned deck (exp009) — verified +12pp over the sample-tuned baseline.
"""
...
from pokemon.heuristic_phase import TwoHKOBenchAgent as _HeuristicAgent
